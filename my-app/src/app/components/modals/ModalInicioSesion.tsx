...
export default function ModalInicioSesion({
  ...
  const handleReenviarCodigo = async () => {
    ...
    setError('');
    try {
      await fetch('https://copia-backend.vercel.app/api/2fa/enviar', {
        method: 'POST',
        headers: {
  ...
  const handleVerify2FA = async () => {
    ...
    try {
      const res = await fetch('https://copia-backend.vercel.app/api/2fa/verificar-login', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${tempToken}`,
      ...
      {!puedeReenviar ? (
        ...
        <>
          <button
            ...
            className={`flex items-left gap-2 my-1 font-semibold w-full h-auto ${intentosReenvio >= MAX_INTENTOS
