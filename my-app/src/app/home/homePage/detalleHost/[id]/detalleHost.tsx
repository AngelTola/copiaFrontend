...
import NavbarSimple from '@/app/components/navbar/NavbarSimple';
...
export default function DetalleHost({ id, comentarios: comentariosIniciales, autos }: Props) {
  ...
  return (
    <>
      {/* Navbar Simple FUERA del contenedor principal */}
      <NavbarSimple />

      {/* Contenido principal */}
      <div className="flex flex-col min-h-screen bg-[var(--background-principal)]">
        <main className="flex-grow max-w-6xl mx-auto p-4 md:p-6">
          ...
          {!cargando && !error && (
            <div>
              ...
              <button
                ...
                disabled={cargando}
                className={`px-5 py-2.5 rounded-full text-base font-semibold transition ${cargando
